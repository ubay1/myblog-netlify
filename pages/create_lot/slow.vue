    ...
    <v-wait for="load_product_slow">
      ...
      <div v-else>
        ...
        <div class="grid-produk" :class={margin_grid_bottom_false:margin_grid_false}>
          <div class="produk" v-for="(item, index) in get_data_slow" :key="index"
            :id="`id-${item.product.id}`"
          >
            <div class="checkme" @click="select_slow(item.product.id, item.product.product_name, item.product.picture[0])">pilih</div>
    ...
    select_slow(id, product_name, picture){
      ...
      if (ell.classList.contains('active')) {
        ...
        if(this.checkedProduct.length >= this.slow){
          this.$swal({
              title: '',
              text: `hanya bisa pilih ${this.slow} product`,
              icon: 'info',
              showCancelButton: false,
            })
          return false
